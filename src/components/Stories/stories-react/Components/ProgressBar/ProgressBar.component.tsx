...
export function ProgressBar(props: IProgressBarProps) {
  ...
  hooks.useAnimationFrame((time: number) => {
    ...
    barWidth =
      Number(
        (barRef.current.style.width || '1px').slice(0, barRef.current.style.width.length - 2)
      ) || 0;
    ...
    step = barWrapperRef?.current?.offsetWidth / ((props.story.duration || defaultDuration) / time);
  ...
  return (
    <div
      ...
      ref={barWrapperRef}
    >
      <div className={`${styles.bar} ${classNames?.progressBar || ''}`} ref={barRef} />
    </div>
  );
}
