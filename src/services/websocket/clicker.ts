...
export const initClicker = (
  ...
  const $levelProgress = computed(
    [$level, $quarks],
    (currentLvl, currentQuarks) => {
      const levelDefIndex = Math.min(
        Math.max(currentLvl - 1, 0),
        LEVELS.length - 1
      )
      ...
      const currentLvlTotalQuarks =
        currentLvlDef.quarksToUpgrade - prevLvlDef.quarksToUpgrade
  ...
  const $energyLimit = computed(
    ...
    (levelDef, upgrades) => {
      ...
      const updatedState = upgradesWithQuarksPerClick.reduce(
        (state, upgrade) => {
    ...
    (energyReset, energyResetAt, energyLimit, time) => {
      ...
      const reduced =
        energyResetAt === 0
          ...
          : Math.max(
              0,
              energyReset + Math.floor(elapsedSeconds * energyRegenRate)
            )
      ...
      case 'upgrade': {
        ...
        if (updatedState.quarks >= 0) {
          ...
          if (currentUpgrade) {
              ...
              if (updatedUpgrade.slug === slug) {
                ...
                updatedUpgrade.prices.map((price, index) => {
                  if (updatedUpgrade.prices && !price) {
                    updatedUpgrade.prices[index] = upgradeDef.price(
                      updatedState,
                      index + 1
                    )
                  }
            ...
            updatedUpgrades = [
              ...updatedUpgrades,
              { slug, tier: 1, prices: [upgradePrice] },
            ]
          }
